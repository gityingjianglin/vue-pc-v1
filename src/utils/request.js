...
import { checkIsLogin, removeLoginInfo } from '@/utils/login'
...
service.interceptors.request.use(config => {
  ...
  if (getStore('clientPmpToken')) {
    config.headers['Authorization'] = 'Bearer ' + getStore('clientPmpToken') // 让每个请求携带自定义token 请根据实际情况自行修改
  }
  // get请求映射params参数
...
service.interceptors.response.use(res => {
  ...
  // debugger
    ...
    if (code === 401) {
      ...
      if(isHaier === 'false') {
        if (!isRelogin.show) {
          removeLoginInfo(config.clientCode)
          isRelogin.show = true;
          MessageBox.confirm('登录状态已过期，您可以继续留在该页面，或者重新登录', '系统提示', {
            ...
            checkIsLogin().then(() => {
              ...
              //   id: getStore(`${clientCode}UserName`),
              //   username: getStore(`${clientCode}NickName`),
              //   email: ''
              // })
            })
          })
        ...
        if (rootVue._router.history.current && rootVue._router.history.current.path === '/') {
          ...
          if (router.history.current.name === 'toDoCenter' && router.history.current.query.from) {
            let query = router.history.current.query
            let obj = {
              path: router.history.current.path,
              query: router.history.current.query
            }
            setStore('todoCenterPageInfo', JSON.stringify(obj))
          }
          // window.location.href = 'https://sit-wlzz.haierfhtech.com/#/index'
